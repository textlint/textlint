...
var debug = require("debug")("textlint");
var messages = [],
    currentText = null;
...
api.lintText = function (text) {
    ...
    var mappingSyntax = require("./parse/plaintext/plaintext-syntax");
    traverse(ast).forEach(function (x) {
        if (this.notLeaf && typeof x.type !== "undefined") {
            var mappedType = mappingSyntax[x.type];
            api.emit(mappedType, x);
        }
    });
    return {
        filePath: "<text>",
...
api.lintMarkdown = function (markdown) {
    ...
    var mappingSyntax = require("./parse/markdown/markdown-syntax");
    traverse(ast).forEach(function (x) {
        if (this.notLeaf && typeof x.type !== "undefined") {
            var mappedType = mappingSyntax[x.type];
            api.emit(mappedType, x);
...
api.pushReport = function (ruleId, txtNode, error) {
    debug("api.pushReport %s to %o", error, txtNode);
    messages.push(objectAssign({
        id: ruleId,
        message: error.message,
        line: error.line ? error.line : txtNode.loc.start.line,
        column: error.column ? error.column : txtNode.loc.start.column,
        severity: 2
    }, txtNode));
};
