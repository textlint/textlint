...
TextLintEngine.prototype.setupRules = function (config) {
    ...
    if (config.rules) {
        ...
        config.rules.forEach(function (ruleName) {
            var realPkgName;
            var pkgPath = tryResolve.relative("textlint-rule-" + ruleName);
            if (pkgPath) {
                realPkgName = "textlint-rule-" + ruleName;
            }else{
                pkgPath = tryResolve.relative(ruleName);
                realPkgName = ruleName;
            }
            if (pkgPath) {
                debug("Loading rules from %s", pkgPath);
                var plugin = require(pkgPath);
                ruleManager.defineRule(realPkgName, plugin);
            } else {
                throw new ReferenceError(ruleName + " is not found --rule");
            }
        });
    }
    textLint.setupRules(ruleManager.getAllRules());
};
