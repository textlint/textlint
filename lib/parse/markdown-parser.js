...
var renderInline = function (inline) {
    ...
    switch (inline.t) {
        ...
        case 'Emph':
            return toPlainText('em', [], this.renderInlines(inline.c));
        case 'Strong':
            return toPlainText('strong', [], this.renderInlines(inline.c));
        case 'Html':
            return inline.c;
        case 'Link':
            ...
            return toPlainText('a', attrs, this.renderInlines(inline.label));
        case 'Image':
            attrs = [
    ...
    for (var i = 0; i < inlines.length; i++) {
        ...
        if (parent != null) {
            objectAssign(inline, {
                ...
                start_column: parent.start_column + result.length
            });
        }
        // render plaintext
        var raw = this.renderInline(inline);
        result = result + raw;

        if (parent != null) {
            var addingLineNumber = (result.split("\n").length - 1);
            objectAssign(inline, {
                end_line: parent.start_line + addingLineNumber,
                raw: raw
            });
        }
    }
