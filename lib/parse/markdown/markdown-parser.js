...
function parse(text) {
    ...
    var rangeNode = require("./markdown-range-node");
    ...
    /*
        compute text length(contain break line) of each line.
        [
            11, // 1st line
            42, // 2nd line
        ]
     */
    var renderLineByLine = text.split("\n");
    var rangeStartLineByLine = (function () {
        var startIndex = 0;
        return renderLineByLine.map(function (line) {
            var result = startIndex + line.length + 1;
            startIndex = result;
            return result;
        })
    })();
    ...
    traverse(ast).forEach(function (x) {
        if (this.notLeaf) {
            ...
            x = objectAssign(x, rangeNode(x, startIndex));
            if (typeof x.t !== "undefined") {
                x.type = x.t;
                delete x.t;
            }
            if (typeof x.loc !== "undefined") {
                // get startIndex from rangeStartLineByLine(0 to ...)
                startIndex = rangeStartLineByLine[x.loc.start.line - 1];
            }
        }
    });
    return ast;
